...
dataset1['sex'] = dataset1[['sex']].replace(['female','male'],[0,1])
dataset1['embarked'] = dataset1[['embarked']].replace(['S','C','Q'],[0,1,2])
dataset = dataset1[['pclass','sex', 'age','sibsp','parch','fare','embarked','survived']]
dataset = dataset.replace('nan', np.nan).dropna()
dataset = dataset.astype(int)

result_col = dataset[['survived']]
data = dataset[['pclass','sex', 'age','sibsp','parch','fare','embarked']].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
        ...
        for l in range(len(fuzz[i][j])):
            ...
            for k in range(len(mylist)):
                if single_features[i][j][l]<=min1[k][j]:
                    c=0
                elif single_features[i][j][l]>min1[k][j] and single_features[i][j][l]<=mid[k][j]:
                    c=(single_features[i][j][l]-min1[k][j])/(mid[k][j]-min1[k][j])
                elif single_features[i][j][l]>mid[k][j] and single_features[i][j][l]<= max1[k][j]:
                    c=((max1[k][j]-single_features[i][j][l])/(max1[k][j]-mid[k][j]))
                else:
                    c=0
                inp.append(c)
            b = pd.DataFrame([inp])
...
#np.random.seed(2)
learning_rate = 0.008810749444691148
#learning_rate = np.random.uniform(0,0.025)
print(learning_rate)
...
testres = test[[result]].replace([0,1], ['Survived','Not Survived'])
