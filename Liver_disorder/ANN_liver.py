...
st_time = time.time()
data = pd.read_csv('liver-disorders.csv')
data = data.replace('?', np.nan).dropna()
data = data.astype(int)

mylist = list(data['selector'].unique())

df_norm = data[['mcv', 'alkphos', 'sgpt', 'sgot','gammagt','drinks']].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
target = data[['selector']].replace([1,2],[0,1])
...
In_train = train.values[:,:6]
y = np.array(train.values[:,6:7])


test = df[df.train == 0]
test = test.drop('train', axis=1).sample(frac=1)

In_test = test.values[:,:6]
y1 = np.array(test.values[:,6:7])
...
testres = test[['selector']].replace([0,1], ['1','2'])
