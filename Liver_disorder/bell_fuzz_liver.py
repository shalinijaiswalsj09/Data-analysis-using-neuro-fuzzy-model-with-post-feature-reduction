...
st_time = time.time()
dataset = pd.read_csv('liver-disorders.csv')
dataset = dataset.replace('?', np.nan).dropna()

dataset = dataset.astype(int)
result_col = dataset[['selector']]
data = dataset[['mcv', 'alkphos', 'sgpt', 'sgot','gammagt','drinks']].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
dataset = pd.concat([data,result_col],axis=1)

name = ['mcv', 'alkphos', 'sgpt', 'sgot','gammagt','drinks']
result = 'selector'
