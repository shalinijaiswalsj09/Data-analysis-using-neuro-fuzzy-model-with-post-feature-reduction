...
result_col = dataset[['Severity']]
data = dataset[['BI-RADS', 'Age', 'Shape', 'Margin','Density']].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
dataset = pd.concat([data,result_col],axis=1)

name = ['BI-RADS', 'Age', 'Shape', 'Margin','Density']
result = 'Severity'
...
y = np.array(train.values[:,(len(train.columns)-1):len(train.columns)])
