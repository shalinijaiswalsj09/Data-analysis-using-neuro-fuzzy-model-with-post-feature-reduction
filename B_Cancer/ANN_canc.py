...
df_norm = data[['clump_thickness', 'unif_cell_size', 'unif_cell_shape', 'marg_adhesion','single_epith_cell_size','bare_nuclei','bland_chrom','norm_nucleoli','mitoses']].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
target = data[['class']].replace([2,4],[0,1])
...
In_train = train.values[:,:9]
y = np.array(train.values[:,9:10])


test = df[df.train == 0]
test = test.drop('train', axis=1).sample(frac=1)

In_test = test.values[:,:9]
y1 = np.array(test.values[:,9:10])
...
testres = test[['class']].replace([0,1], ['Benign','Malignant'])
