...
data1 = pd.read_csv('hepatitis.csv')
data1 = data1.astype(int)
row = data1.shape[0]
column = data1.shape[1]
target = data1[['class']]
#df_norm = data[:,:(column-1)]
df_norm = data1[['age','sex','steroid','antivirals','fatigue','malaise','anorexia','liver_big','liver_firm','spleen_palable','spiders','ascites','varices','bilirubin','alk_phosphate','sgot','albumin','protime','histology']]
...
result_col = data1[['class']]
data = data1[['sex','age','protime','liver_big','malaise']].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
dataset = pd.concat([data,result_col],axis=1)


name = ['sex','age','protime','liver_big','malaise']
result = 'class'
