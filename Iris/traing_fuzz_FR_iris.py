...
result_col = dataset[['Species']]
data = dataset[['SepalLengthCm', 'SepalWidthCm', 'PetalLengthCm', 'PetalWidthCm']].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
...
#np.random.seed(3)
learning_rate = 0.440638
#learning_rate = np.random.uniform(0,0.8)
print(learning_rate)
