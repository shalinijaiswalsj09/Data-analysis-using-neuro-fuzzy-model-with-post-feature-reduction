...
mylist = list(data['col1'].unique())

df_norm = data[['col2','col3','col4','col5','col6']].apply(lambda x: (x - x.min()) / (x.max() - x.min()))

target = data[['col1']].replace([1,2,3],[0,1,2])
...
plt.ylabel("Error")
plt.title("Variation of Error with No. of Epochs (ANN)")
plt.show()  
'''
epoch1 = list(range(epoch+1))
ax = plt.subplot()
...
X1 = test.values[:,:5]
y1 = list(map(int,(np.array(test.values[:,5:6]))))
y2 = np.array([targets[int(x)] for x in test.values[:,5:6]])

l1 = 1/(1 + np.exp(-(np.dot(X1, w1))))
l2 = 1/(1 + np.exp(-(np.dot(l1, w2))))

yp = list(map(int,(np.argmax(l2, axis=1)))) # prediction
...
testres = test[['col1']].replace([0,1,2], [1,2,3])
