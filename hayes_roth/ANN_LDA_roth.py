...
df_norm = data[['name', 'hobby', 'age', 'educational level','marital status']].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
...
#np.random.seed(2)
learning_rate = 0.045146048482711255
...
ax = plt.subplot()
p1, =plt.plot(epoch1,er, '-.', color='black', label= 'ANN with PCA', lw=2)
