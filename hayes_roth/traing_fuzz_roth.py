...
st_time = time.time()
dataset = pd.read_csv('hayes_roth.csv')

result_col = dataset[['class']]
data = dataset[['name', 'hobby', 'age', 'educational level','marital status']].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
dataset = pd.concat([data,result_col],axis=1)
name = ['name', 'hobby', 'age', 'educational level','marital status']
result = 'class'
