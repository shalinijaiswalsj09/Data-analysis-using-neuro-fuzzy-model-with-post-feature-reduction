...
df_norm = data[['age', 'sex', 'chest', 'resting_blood_pressure','serum_cholestoral','fasting_blood_sugar','resting_electrocardiographic_results','maximum_heart_rate_achieved','exercise_induced_angina','oldpeak','slope','number_of_major_vessels','thal']].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
...
In_train = train.values[:,:12]
y = np.array(train.values[:,12:13])


test = df[df.train == 0]
test = test.drop('train', axis=1).sample(frac=1)

In_test = test.values[:,:12]
y1 = np.array(test.values[:,12:13])
...
testres = test[['class']].replace([0,1], ['0','1'])
